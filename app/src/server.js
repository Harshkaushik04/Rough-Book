...
io.sockets.on('connect', async (socket) => {
     ...
     * Text Editor actions
     */
    socket.on('textEditorAction', async (cfg) => {
         // Prevent XSS injection
         const config = checkXSS(cfg);
         log.debug('Text Editor Action', config);
         const { room_id } = config;
         // Broadcast to all other peers in the room
         await sendToRoom(room_id, socket.id, 'textEditorAction', config);
    });

    socket.on('textEditorUpdate', async (cfg) => {
         // Prevent XSS injection
         const config = checkXSS(cfg);
         // Optional: Avoid logging every keystroke to prevent log spam
         // log.debug('Text Editor Update', config); 
         const { room_id } = config;
         // Broadcast to all other peers in the room
         await sendToRoom(room_id, socket.id, 'textEditorUpdate', config);
    });
